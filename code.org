...
RUN wget --no-check-certificate\
    https://repo.continuum.io/miniconda/Miniconda3-latest-Linux-x86_64.sh \
    && bash Miniconda3-latest-Linux-x86_64.sh -b -p /home/user/conda \
    && rm -f Miniconda3-latest-Linux-x86_64.sh 
...
#RUN echo PROJ_LIB="/usr/share/proj" > ~/.bashrc
...
#+BEGIN_SRC docker 
#RUN git clone -c http.sslverify=false https://github.com/OSGeo/grass-addons /usr/local/grass/addons/.

# RUN echo "GRASS_ADDON_PATH=/usr/local/grass/addons/" > ~/.bashrc
# RUN echo "export GRASS_ADDON_PATH"

#RUN echo ${GRASS_ADDON_PATH}
...
RUN echo LANG="en_US.UTF-8" > /etc/default/locale
RUN python -c "import pandas as pd; print(pd)" 
...
docker run -it --user "$(id -u)":"$(id -g)" --mount type=bind,src="${DATADIR}",dst=/data --mount type=bind,src="$(pwd)",dst=/home/user --env PARALLEL="--delay 0.1 -j -1" mass_balance grass -e -c EPSG:4326 G_HIRHAM

docker run -it --user "$(id -u)":"$(id -g)" --mount type=bind,src="${DATADIR}",dst=/data --mount type=bind,src="$(pwd)",dst=/home/user --env PARALLEL="--delay 0.1 -j -1" mass_balance python -c 'import pandas as pd; print(pd)' 
