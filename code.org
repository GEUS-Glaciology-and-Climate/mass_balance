...
this_D = xr.open_dataset('/home/kdm/data/Mankoff_2020/ice/gate.nc')\
           .resample({'time':'YS'})\
           .mean()\
           .reindex(time=promice['time'])
...
# graphics
axD.scatter(promice['D'].sum(dim='sector').values,
            this_D['discharge'].sum(dim='gate').values)
axD.plot([400,500],[400,500], 'k-', alpha=0.25)
axD.set_ylabel('D [Gt; this]')
axD.set_xlabel('D [Gt; Colgan (2019)]')


axSMB.scatter(promice['SMB'].sum(dim='sector').values,
              this_smb['MAR_Z'].sum(dim='sector').values)
axSMB.plot([100,600],[100,600], 'k-', alpha=0.25)
axSMB.set_ylabel('SMB [Gt; this]')
axSMB.set_xlabel('SMB [Gt; Colgan (2019)]')



axMB.scatter(promice['MB'].sum(dim='sector').values,
             this_mb['MAR_sector'].sum(dim='sector').values)
axMB.plot([-300,0],[-300,0], 'k-', alpha=0.25)
axMB.set_ylabel('MB [Gt; this]')
axMB.set_xlabel('MB [Gt; Colgan (2019)]')
