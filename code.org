...
e = axMB.errorbar(mb, k2015['mb_orig'], xerr=mmb['mmb_err'], yerr=k2015['mmb_err'], **kw_errbar)
...
axMB.set_ylabel('MB$^{*}$\n[Gt yr$^{-1}$]', labelpad=-25)
axMB.set_xlabel('This Study (recent)\n[Gt yr$^{-1}$]')
...
axMB.set_yticklabels([str(ax.get_xticks()[0]), '', str(ax.get_xticks()[-1])])
...
axCB = add_sub_axes(axMB, [0.0, -0.65, 1, 0.075])
...
#+RESULTS:
: <ipython-input-7-e356df1c5520>:328: UserWarning: This figure includes Axes that are not compatible with tight_layout, so results might be incorrect.
:   plt.savefig('fig/K2015_adjusted.png', transparent=False, bbox_inches='tight', dpi=300)
...
axins = ax.inset_axes([0.15, 0.08, 0.7, 0.42])
...
# OFFSET = this['mb'].resample('YS').mean().max()
OFFSET = -400
...
vc_sum = vc_sum - np.min(vc_sum) + -5200
...
legend = plt.legend(lines, label, framealpha=1, loc='upper right', bbox_to_anchor=(0.95,1), ncol=1, facecolor='w')
...
#+RESULTS:
: <ipython-input-132-b7430c004235>:172: UserWarning: This figure includes Axes that are not compatible with tight_layout, so results might be incorrect.
:   plt.savefig('fig/mouginot_2019.png', transparent=False, bbox_inches='tight', dpi=300)
...
# axCB = add_sub_axes(axMB, [1.1, 0.0, 0.075, 1])
axCB = add_sub_axes(axMB, [0.0, -0.70, 1, 0.075])
cb = plt.colorbar(s, cax=axCB, orientation='horizontal', ticks=[0,1])
cb.set_alpha(1) # https://stackoverflow.com/questions/4478725/
cb.draw_all()
cb.ax.xaxis.set_ticks_position('top')
...
#+RESULTS:
: <ipython-input-35-e48d76cbd9de>:164: UserWarning: This figure includes Axes that are not compatible with tight_layout, so results might be incorrect.
:   plt.savefig('fig/colgan_2019.png', transparent=False, bbox_inches='tight', dpi=300)
...
axGRACE.set_ylabel('GMB\n[Gt yr$^{-1}$]', labelpad=-25)
axVC.set_ylabel('VC\n[Gt yr$^{-1}$]', labelpad=-25)
axIMBIE.set_ylabel('IMBIE\n[Gt yr$^{-1}$]', labelpad=-25)
axIMBIE.set_xlabel('This Study\n[Gt yr$^{-1}$]', labelpad=0)
...
: <ipython-input-2-e13bd9bca3cc>:209: UserWarning: This figure includes Axes that are not compatible with tight_layout, so results might be incorrect.
:   plt.savefig('fig/this_v_grace_vc_imbie.png', transparent=False, bbox_inches='tight', dpi=300)
...
ax.set_ylabel('BMB$_{\mathrm{VHD}}$\n[Gt yr$^{-1}$]', labelpad=-15)
...
# axCB = add_sub_axes(axMB, [1.1, 0.0, 0.075, 1])
axCB = add_sub_axes(ax, [0.0, -0.43, 1, 0.075])
cb = plt.colorbar(scatter, cax=axCB, orientation='horizontal', ticks=[0,1])
cb.set_alpha(1) # https://stackoverflow.com/questions/4478725/
cb.draw_all()
cb.ax.xaxis.set_ticks_position('top')
...
|------------------------+-----+----+----+----+----+-----+----+------+----+----+----+----+-----+-----+-----|
| #ERROR                 |  33 | 19 | 19 | 11 | 14 |  14 | 14 |   12 | 10 |  8 | 10 | 13 |   6 |   5 |  11 |
#+TBLFM: @>=vsum(@2..@-1)
...
Export via ODT (works OK) or...

#+BEGIN_SRC bash
pandoc -f latex -t docx -o ms.docx --citeproc --bibliography ./library.bib ms.tex
#+END_SRC
...
k = pd.read_csv(datadir+'Kjeldsen_2015/Greenland_mass_balance_totals_1840-2012_ver_20141130_with_uncert_via_Kjeldsen_et_al_2015.csv', **kw)
