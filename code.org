...
D['D'] = (('time'), D['D_sector'].sum(dim='sector'))
...
#+RESULTS: load_D
*** Forecast D for sectors and regions
...
df = D[['D','D_err']].to_dataframe()

# select last 3 years for linear trend
df3 = df[-365*3:]

# short term variations
df3['seasonal'] = sp.signal.detrend(df3['D'].values)
...
# forecast['D_err'] = df3.iloc[-1]['D_err'] + forecast['err'].cumsum()
forecast['D_err'] = df3.iloc[-1]['D_err'] + forecast['err']

df = df.reindex(time_all).bfill()
df.loc[time_last_obs:, 'D'] = forecast[time_last_obs:]['best']
df.loc[time_last_obs:, 'D_err'] = forecast[time_last_obs:]['D_err']
#+END_SRC

*** INPROGRESS Forecast D for GIS: xarray

+ D:
  + Estimate +7d D from long term trend + seasonal trend
...
D_all = D.reindex({'time':time_index_all})
...
for v in ['D','D_region','D_sector']:
    # trend is linear long term trend of last 3 years of observations.
    last3_plus_forecast = D_all[v].sel({'time':slice(time_index_last3y[0],time_forecast_end)})
    ...
    D_all[v] = xr.concat([D[v],forecast], dim='time').interp(time=time_index_all)
    
for v in ['D_err','err_region','err_sector']:
    last3_hindcast = D_all[v].sel({'time':time_index_hindcast})
    ...
    D_all[v] = D_all[v].ffill(dim='time') + e.cumsum().reindex({'time':time_index_all}).fillna(0)

D_all = D_all.bfill(dim='time')
...
# D_all['D'].plot(ax=ax)
# ax.fill_between(D_all['time'],
#                 D_all['D']-D_all['D_err'],
#                 D_all['D']+D_all['D_err'],
#                 color='gray', alpha=0.25)

r = 'NO'
D_all['D_region'].sel({'region':r}).plot(ax=ax)
ax.fill_between(D_all['time'],
                D_all['D_region'].sel({'region':r}) - D_all['err_region'].sel({'region':r}),
                D_all['D_region'].sel({'region':r}) + D_all['err_region'].sel({'region':r}),
                color='gray', alpha=0.25)
...
*** DONE Forecast D for GIS: Pandas
...
D['D_err'] = (('time'),
                  np.hstack((k2015['D_err'].values/365,
                             D['D_err'].sel({'time':slice('1986',dt_last_obs)}).values,
                             err[1:])).ravel())
...
for roi in ['sector', 'region']: # TODO: 'basin'
    ...
    MB['MB_err'] = (('time'), (SMB['SMB_err']**2 + D['D_err']**2 + BMB['BMB_err']**2)**0.5)
    ...
    MB['MB_ROI'] = (('time',roi), SMB['SMB_'+roi] - D['D_'+roi] - BMB['BMB_'+roi])
    MB['MB_ROI_err'] = (('time',roi), (SMB['SMB_'+roi+'_err']**2 + D['err_'+roi]**2 + BMB['BMB_'+roi+'_err']**2)**0.5)
    ...
    MB['D'] = (('time'), D['D'])
    MB['D_err'] = (('time'), D['D_err'])
    MB['D_ROI'] = (('time',roi), D['D_'+roi])
    MB['D_ROI_err'] = (('time',roi), D['err_'+roi])
    ...
    for RCM in ['HIRHAM','MAR','RACMO']:
        MB['MB_'+RCM] = (('time',roi), SMB[RCM+'_'+roi]-D['D_'+roi]-BMB['BMB_'+roi] )
        v = 'MB_'+RCM
...
axD.text(0, 0.9, 'D [+BMB]', transform=axD.transAxes)
axD.set_xticks([375,600])
...
+ Paper: Accepted
+ Data: doi:10.22008/FK2/OHI23Z or https://doi.org/10.22008/FK2/OHI23Z
+ Code: https://github.com/GEUS-Glaciology-and-Climate/mass_balance

+ Contact: kdm (at) geus (dot) dk
#+END_SRC
...
** Twitter figure
:PROPERTIES:
:header-args:jupyter-python+: :tangle twitfig.py
:END:
...
** Twitter bot
:PROPERTIES:
:header-args:jupyter-python+: :tangle twitbot.py
:END:
...
tweet_str += '\n\n'
tweet_str += '  https://doi.org/10.5194/essd-2021-131'
...
: 
:   Since 1990 (IPCC FAR): 14.75 ±7.69 mm (-5338 Gt)
: 
:   2021 (→️ 2021-09-08): 0.7 ±0.17 mm (-255 Gt)
: 
:   Next 7 days (2021-09-09 →️ 2021-09-15)📉: mass gain +17.7 ±2.8 Gt
: 
