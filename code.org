...
bmb['bmb'] = (('time'), bmb['GF_sector'].sum(dim='sector') \
...
bmb['bmb_err'] = (('time'), ((bmb['GF_sector_err']**2 + bmb['vel_sector_err']**2 + bmb['VHD_sector_err']**2)**0.5).sum(dim='sector'))
...
# dfK["SMB"]=dfK.accumulation-dfK.runoff

box['mmb'] = box['discharge from 6 year lagged average runoff']
box['mmb_err'] = box['discharge 1sigma']
box['mb'] = box['TMB']
box['mb_err'] = box['TMB 1sigma']
box['smb'] = box['accumulation'] - box['runoff']
box['smb_err'] = (box['accumulation 1sigma']**2 + box['runoff 1sigma']**2)**0.5
...
smb['smb_err'] = (('time'),
                  np.append(box['smb_err'].values/365, smb['smb_err'].sel({'time':slice('1986','2200')}).values))
...
mmb['mmb_err'] = (('time'),
                  np.hstack((box['mmb_err'].values/365,
...
smb = (365*mb[['smb','smb_err']]).to_dataframe()\
                                 .resample('AS')\
                                 .mean()
...
mmbbmb = (-365*mb[['mmb','bmb','mmb_err','bmb_err']]).to_dataframe()\
                                                     .resample('AS')\
                                                     .mean()
...
mb_ann = 365 * mb[['mb','mb_err']].to_dataframe()\
                                  .resample('AS')\
                                  .mean()
...
M2021 = mb['mb'].to_dataframe(name='mb').resample('1D').ffill().cumsum()
M2021['err'] = mb['mmb_err'].to_dataframe(name='err')[::-1].cumsum()[::-1]
M2021.loc[:'1985'] = M2021.loc[:'1985'].resample('Y').mean().resample('1D').bfill()

# OFFSET = M2021['mb'].resample('YS').mean().max()
OFFSET = -400
M2021['mb'] = M2021['mb'] - OFFSET

tt = (M2021.index.max() - M2021.index) / (M2021.index.max() - pd.Timestamp('2000-01-01'))
tt = tt.where(tt <= 1, 1)

M2021['bumpy'] = M2021['mb']
M2021['smooth'] = M2021['bumpy'].rolling(365).mean()
M2021['err_core'] = (M2021['bumpy'] * (1-tt)) + (M2021['smooth'] * tt)
p_M2021 = M2021['mb'].plot(ax=ax, color=C_M2021, drawstyle='steps-post', **kw)
...
(M2019-M2019_err*0.75).plot(ax=ax, color=C_M2019, alpha=0.25, linestyle='--', **kw)
(M2019+M2019_err*0.75).plot(ax=ax, color=C_M2019, alpha=0.25, linestyle='--', **kw)
...
kw_err['alpha'] = 0.33
ax.fill_between(P.index.values, (P-P_err*0.75).values.flatten(), (P+P_err*0.75).values.flatten(), color = C_PROMICE, **kw_err)
...
<<load_box>>
box = box.loc['1986':'2012'].rename(columns={'TMB 1sigma':'err'})[['TMB','err']]
...
s = my_plot(axBOX,  this_mb_box['mb'], this_mb_box['mb_err'], box['TMB'], box['err'])
