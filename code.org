...
#+BEGIN_SRC jupyter-python
import xarray as xr
ds = xr.open_mfdataset('/home/kdm/data/MAR/3.12/MAR.nc', combine='by_coords')['ru']
ds = ds.sel({'sector':1}).sum(dim='sector').resample({'time':'YS'}).mean()
...
ds = ds.sum(dim='time').sel({'sector':1})
