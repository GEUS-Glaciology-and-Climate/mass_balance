...
for f in $(ls ${dir}/*.nc | tail -n 5); do ## operational
...
time = pd.date_range(start = "1986-01-01",
                     end = datetime.datetime.utcnow().date() + datetime.timedelta(days = 10),
                     freq = "D")
...
smb['smb_err'] = (('time'), smb['smb_sector'].sum(dim='sector') * 0.08)
...
The 1840 through 2012 runoff comes from an [[mu4e:msgid:AM5PR04MB309006FAB95A2D37B1FCF497B46B9@AM5PR04MB3090.eurprd04.prod.outlook.com][email]]

#+BEGIN_QUOTE
From: Kristian Kjellerup Kjeldsen
To: Ken Mankoff
Date: Tue 16 Mar 2021 01:39:18 PM PDT
Attachments: [1] Runoff_v3.xlsx(30.7K)
#+END_QUOTE

And is saved as [[./dat/historical_runoff.csv]] with some header changes

#+BEGIN_SRC bash :results verbatim
md5sum ./dat/historical_runoff.csv
#+END_SRC

#+RESULTS:
: 29972813de2196042c585ae95614b65e  ./dat/historical_runoff.csv
...
To: Ken Mankoff
Cc: Jason Eric Box
Date: Tue 16 Mar 2021 04:05:12 AM PDT
Attachments: [1] TMB_GrIS_all_Export.xlsx (45.2K), [2] TMB_160321.pdf (589.9K)
#+END_QUOTE

Then saved as CSV with columns renamed in [[./dat/historical_SMB_D_MB.csv]]
...
### Adjusted data from Kjeldsen email
# fname = 'historical_SMB_D_MB.csv'
# kb = pd.read_csv('./dat/' + fname, index_col=0, parse_dates=True)
# kb.index = [pd.to_datetime(str(_) + '-01-01') for _ in kb.index.year]
# kb = kb.rename(columns={'SMB' : 'KB_smb',
#                                   'SMB err' : 'KB_smb_err',
#                                   'D 6yr' : 'KB_mmb',
#                                   'D err' : 'KB_mmb_err'})
# kb = kb[['KB_smb','KB_smb_err','KB_mmb','KB_mmb_err']]

### Published data
fname = 'Greenland_mass_balance_totals_1840-2012_ver_20141130_with_uncert_via_Kjeldsen_et_al_2015.csv'
kb = pd.read_csv('/home/kdm/data/Box_2013/' + fname, index_col=0, parse_dates=True)\
    .rename(columns={'discharge from 6 year lagged average runoff' : 'mmb',
                     'discharge 1sigma' : 'mmb_err'})

kb.index.name = 'time'

kb['smb'] = kb['accumulation'] - kb['runoff']
kb['smb_err'] = (kb['accumulation 1sigma']**2 + kb['runoff 1sigma']**2)**0.5
# kb['mb_no_bmb'] = kb['smb'] - kb['mmb']
# kb['mb_no_bmb_err'] = (kb['smb_err']**2 + kb['mmb_err']**2)**0.5

kb = kb.drop(columns=['accumulation', 'accumulation 1sigma', 'melt', 'melt 1sigma', 'retention',
                      'retention 1sigma',
...
fig = plt.figure(1, figsize=(3.26,7.25)) # w,h
fig.clf()
...
def my_plot(ax, x, xerr, y, yerr):
    ...
    for i,s in enumerate(y2str):
        ...
        if ax == axSMB: BIAS = SMB_BIAS
        if ax == axD: BIAS = MMB_BIAS
        if ax == axMB: continue
        ax.text(x.values[i], y.values[i] - BIAS, '.',
                c = cmap(color)[i],
                fontsize=15,
                fontweight='bold',
                horizontalalignment='center',
    ...
    if ax == axSMB:
        ...
        ax.set_xticks([0,600])
        ax.set_ylabel('KB (historical)\n[Gt yr$^{-1}$]', labelpad=-25)
    if ax == axD:
        ax.text(0, 0.9, 'MMB', transform=ax.transAxes)
        ax.set_xticks([375,600])
        # ax.set_xticks([0,600])
    if ax == axMB:
        ax.text(0, 0.9, 'MB [+BMB]', transform=ax.transAxes)
        ax.set_xticks([-450, 0, 150])
        ax.set_xlabel('This Study (recent)\n[Gt yr$^{-1}$]')
    ...
    if ax == axMB: arrow = ""
    s = "r$^2$: %.2f\nbias: %.1f"+arrow+"\nrmse: %.1f"
    ax.text(1.0, -0.03, s % (round(r_value**2,2), round(bias,1), round(rmse,2)),
...
my_plot(axSMB, smb['smb'], smb['smb_err'], kb['smb'], kb['smb_err'])
my_plot(axD,   mmb['mmb'], mmb['mmb_err'], kb['mmb'], kb['mmb_err'])
s = my_plot(axMB,
            smb['smb']-mmb['mmb'],
            (smb['smb_err']**2+mmb['mmb_err']**2)**0.5,
            (kb['smb']-SMB_BIAS) - (kb['mmb']-MMB_BIAS),
            (kb['smb_err']**2+kb['mmb_err']**2)**0.5)
...
axCB = add_sub_axes(axMB, [0.0, -0.8, 1, 0.075])
...
axCB.set_xticklabels(kb.index.year[[0,-1]].values)

plt.savefig('fig/KB_adjusted.png', transparent=False, bbox_inches='tight', dpi=300)
#+END_SRC

#+RESULTS:
: <ipython-input-64-49391b7caaa1>:212: UserWarning: This figure includes Axes that are not compatible with tight_layout, so results might be incorrect.
:   plt.savefig('fig/KB_adjusted.png', transparent=False, bbox_inches='tight', dpi=300)
...
** Load and adjust Historical (KB)

#+NAME: load_and_adjust_KB
#+BEGIN_SRC jupyter-python
<<load_kb>>
...
kb['smb'] = kb['smb'] - SMB_BIAS
kb['mmb'] = kb['mmb'] - MMB_BIAS
...
vr = v.merge(kb['runoff'], left_index=True, right_index=True)
...
kb['bmb_GF'] = bmb_C['GF_sector']*365
kb['bmb_GF_err'] = bmb_C['GF_sector_err']*365
kb['bmb_vel'] = bmb_C['vel_sector']*365
kb['bmb_vel_err'] = bmb_C['vel_sector_err']*365

kb['bmb_VHD'] = (kb['runoff']) * slope
kb['bmb_VHD_err'] = (kb['runoff 1sigma']) * slope

kb[[_ for _ in kb.columns if '_' in _ ]].head()

kb['bmb'] = kb[['bmb_GF','bmb_vel','bmb_VHD']].sum(axis='columns')
kb['bmb_err'] = (kb['bmb_GF']**2 \
                  + kb['bmb_vel']**2 \
                  + kb['bmb_VHD']**2)**0.5

kb['mb'] = kb['smb'] - kb['mmb'] - kb['bmb']
#+END_SRC

#+RESULTS: load_and_adjust_KB
...
kb = kb.loc['1840':'1986']
# kb = kb.loc['1840':'1986'].resample('1D').ffill().iloc[:-1] # cut off 1986-01-01
kb = kb.loc['1840':'1986'].iloc[:-1] # cut off 1986-01-01



<<load_smb>>
time = np.append(kb.index, smb['time'].values)
smb = smb.reindex({'time':time})
smb['smb'] = (('time'),
              np.append(kb['smb'].values/365, smb['smb'].sel({'time':slice('1986','2200')}).values))
smb['smb_err'] = (('time'),
                  np.append(kb['smb_err'].values/365, smb['smb_err'].sel({'time':slice('1986','2200')}).values))
...
mmb['mmb'] = (('time'),
              np.append(kb['mmb'].values/365, mmb['mmb'].sel({'time':slice('1986','2200')}).values))
mmb['mmb_err'] = (('time'),
                  np.hstack((kb['mmb_err'].values/365,
...
bmb['bmb'] = (('time'),
              np.append(kb['bmb'].values/365, bmb['bmb'].sel({'time':slice('1986','2200')}).values))
bmb['bmb_err'] = (('time'),
                  np.append(kb['bmb_err'].values/365, bmb['bmb_err'].sel({'time':slice('1986','2200')}).values))
...
for roi in ['sector', 'region']: # TODO: 'basin'
    ...
    mb_hist = (kb['smb'] - kb['mmb'] - kb['bmb'])/365
...
smb = mb[['smb','smb_err']]\
    ...
    .resample('AS')\
    .sum()
smb['smb'].plot(color='b', linestyle=':', **kw)
...
(-1*(mb['mmb'] + mb['bmb'])).to_dataframe(name='M2021')\
...
** NOTDONE This recent vs. historical GIS xy
...
# df_1840 from modified Kjeldsen (2015)
<<load_kjeldsen>>
df_1840 = df_1840.loc['1986':'2012']

mb = xr.open_dataset('./TMB/mb_region.nc')\
       .resample({'time':'YS'})\
       .sum()\
       .reindex(time=df_1840.index)
...
def my_plot(ax, x, xerr, y, yerr):
    ...
    rmse = np.sqrt(np.mean((x.values - y.values)**2))
    # ax.plot(x, slope * x + intercept, 'k', linewidth=0.5)
    ax.text(1.0, -0.03, "r$^2$: %.2f\nbias: %.1f\nrmse: %.1f" % (round(r_value**2,2), round(bias,1), round(rmse,2)),
            transform=ax.transAxes,
            horizontalalignment='right',
            fontsize=9)
...
my_plot(axSMB, mb['smb'], mb['smb_err'], df_1840['smb'], df_1840['smb_err'])
my_plot(axD,   mb['mmb']+mb['bmb'], (mb['mmb_err']**2+mb['bmb_err']**2)**0.5, df_1840['D'], df_1840['D_err'])
s = my_plot(axMB,  mb['mb'], mb['mb_err'], df_1840['mb'], df_1840['mb_err'])
...
cb.ax.xaxis.set_label_position('top')
# cb.set_label("Time [year]", labelpad=-20)
axCB.set_xticklabels(df_1840.index.year[[0,-1]].values)

plt.savefig('fig/kjeldsen_2015.png', transparent=False, bbox_inches='tight', dpi=300)
#+END_SRC
...
: <ipython-input-57-c609bb5b98c1> in <module>
:      24 # df_1840 from modified Kjeldsen (2015)
:      25 
: ---> 26 df_1840 = df_1840.loc['1986':'2012']
:      27 
:      28 mb = xr.open_dataset('./TMB/mb_region.nc')\
: 
: NameError: name 'df_1840' is not defined
:END:
...
my_plot(axD, mb['mmb']+mb['bmb'], (mb['mmb_err']**2+mb['bmb_err']**2)**0.5, mouginot['D'], mouginot['D_err'])
...
#+RESULTS:
: <ipython-input-12-e2b694aa7a6b>:145: UserWarning: This figure includes Axes that are not compatible with tight_layout, so results might be incorrect.
:   plt.savefig('tmp/mouginot_2019.png', transparent=False, bbox_inches='tight', dpi=300)
...
my_plot(axD,   mb['mmb']+mb['bmb'], (mb['mmb_err']**2+mb['bmb_err']**2)**0.5, promice['D'], promice['D_err'])
...
#+RESULTS:
: <ipython-input-6-7ee885e94103>:151: UserWarning: This figure includes Axes that are not compatible with tight_layout, so results might be incorrect.
:   plt.savefig('tmp/colgan_2019.png', transparent=False, bbox_inches='tight', dpi=300)
...
del(grace)
if 'grace' not in locals():
    <<load_grace>>
    # g2 = grace.copy(deep=True)
    # grace = grace.resample('1D').mean().interpolate(method='time').resample('YS').mean().diff().dropna()
    # grace = grace.diff().dropna().resample('YS').sum()
    grace = (grace.resample('1D').mean().interpolate(method='time')/365).resample('YS').sum().diff()
...
def my_plot(ax, x, xerr, y, yerr):
    ...
    ax.text(1.0, -0.03,
            "r$^2$: %.2f\nbias: %.1f\nrmse: %.1f"
            % (round(r_value**2,2), round(bias,1), round(rmse,2)),
            transform=ax.transAxes,
            horizontalalignment='right',
            fontsize=9)
...
s = my_plot(axGRACE, this_mb_grace['mb'], this_mb_grace['mb_err'], grace['mass'], grace['err'])
s = my_plot(axSEC, this_mb_sec['mb'], this_mb_sec['mb_err'], sec['SEC'], sec['err'])
s = my_plot(axIMBIE,  this_mb_imbie['mb'], this_mb_imbie['mb_err'], imbie['mb'], imbie['mb_err'])

plt.savefig('fig/this_v_grace_sec_imbie.png', transparent=False, bbox_inches='tight', dpi=300)
#+END_SRC
...
: <ipython-input-123-d5f1d6d7abec>:191: UserWarning: This figure includes Axes that are not compatible with tight_layout, so results might be incorrect.
:   plt.savefig('fig/this_v_grace_sec_imbie.png', transparent=False, bbox_inches='tight', dpi=300)
...
** INPROGRESS ROIs, RCMS, and extended ROIs

#+BEGIN_SRC bash
grass -c ./G_RACMO/ROI_e

g.region region=RCM -pa

d.mon wx0
d.erase

d.rast mask_ice
d.vect sectors@Zwally_2012 color=blue fill_color=none
# d.vect sectors_e@Zwally_2012 color=blue fill_color=none

d.vect basins@Mouginot_2019 color=red fill_color=none
# d.vect regions_e@Mouginot_2019 color=red fill_color=none
...
this_smb['Z'] = (('sector'), (this_smb.sector.values/10).astype(int))
this_smb = this_smb.groupby('Z').sum().rename({'Z':'sector'})
this_D['Z'] = (('sector'), (this_D.sector.values/10).astype(int))
this_D = this_D.groupby('Z').sum().rename({'Z':'sector'})
this_mb['Z'] = (('sector'), (this_mb.sector.values/10).astype(int))
...
** DONE Mankoff 2020 mmb vs. 6yr avg runoff

Comparison data provenance:

+ [[mu4e:msgid:AM5PR04MB309066C23A108AEF8D386F64B46B9@AM5PR04MB3090.eurprd04.prod.outlook.com][link to email]]

#+BEGIN_QUOTE
From: Kristian Kjellerup Kjeldsen
To: Ken Mankoff
Date: Tue 16 Mar 2021 08:27:30 AM PDT
Attachments: SID vs 6ave runoff.xlsx (11.9K)
#+END_QUOTE

That attachment saved as [[./dat/D_vs_6yr_runoff.csv]] with column headers changed.

#+BEGIN_SRC bash :results verbatim
md5sum ./dat/D_vs_6yr_runoff.csv
#+END_SRC

#+RESULTS:
: 7f447034ced2060fd73d8d0c8984aca3  ./dat/D_vs_6yr_runoff.csv
...
# plt.close(1)
fig = plt.figure(1, figsize=(3.26,3)) # w,h
fig.clf()
fig.set_tight_layout(True)
ax = fig.add_subplot(111)

df = pd.read_csv('./dat/D_vs_6yr_runoff.csv', parse_dates=True, index_col=0)
...
s = my_plot(ax, df['6yr_runoff'], df['6yr_runoff']*0, df['D'], df['D']*0)
...
# axCB = add_sub_axes(axMB, [1.1, 0.0, 0.075, 1])
axCB = add_sub_axes(ax, [0.0, -0.75, 1, 0.075])
cb = plt.colorbar(s, cax=axCB, orientation='horizontal', ticks=[0,1])
cb.set_alpha(1) # https://stackoverflow.com/questions/4478725/
cb.draw_all()
cb.ax.xaxis.set_ticks_position('top')
cb.ax.xaxis.set_label_position('top')
# cb.set_label("Time [year]", labelpad=-20)
axCB.set_xticklabels(df.index.year[[0,-1]].values)

plt.savefig('fig/mmb_v_6yr.png', transparent=False, bbox_inches='tight', dpi=300)
#+END_SRC

#+RESULTS:
: <ipython-input-178-6cf82cd001b2>:92: UserWarning: This figure includes Axes that are not compatible with tight_layout, so results might be incorrect.
:   plt.savefig('fig/mmb_v_6yr.png', transparent=False, bbox_inches='tight', dpi=300)
...
** DONE Mankoff 2020 runoff vs . 6yr avg runoff
...
# plt.close(1)
fig = plt.figure(1, figsize=(3.26,5)) # w,h
fig.clf()
fig.set_tight_layout(True)
axMAR = fig.add_subplot(211)
axRACMO = fig.add_subplot(212)

df = pd.read_csv('./dat/D_vs_6yr_runoff.csv', parse_dates=True, index_col=0)

if 'RACMO' not in locals():
    RACMO = xr.open_mfdataset('/home/kdm/data/Mankoff_2020/water/ice/runoff/RACMO*.nc')['runoff']\
          .sel({'time':slice('1986','2012')})\
          .sum(dim='station')\
          .resample({'time':'YS'})\
          .mean()\
          .to_dataframe()

    MAR = xr.open_mfdataset('/home/kdm/data/Mankoff_2020/water/ice/runoff/MAR*.nc')['runoff']\
          .sel({'time':slice('1986','2012')})\
          .sum(dim='station')\
          .resample({'time':'YS'})\
          .mean()\
...
def my_plot(ax, x, xerr, y, yerr):
    ...
    if ax == axMAR:
        ax.set_ylabel('MAR runoff\n[Gt yr$^{-1}$]', labelpad=-25)
    else:
        ax.set_ylabel('RACMO runoff\n[Gt yr$^{-1}$]', labelpad=-25)
        ax.set_xlabel('KB runoff\n[Gt yr$^{-1}$]')
...
s = my_plot(axRACMO, df['6yr_runoff'], df['6yr_runoff']*0, RACMO['runoff'], RACMO['runoff']*0)
s = my_plot(axMAR, df['6yr_runoff'], df['6yr_runoff']*0, MAR['runoff'], MAR['runoff']*0)
...
# axCB = add_sub_axes(axMB, [1.1, 0.0, 0.075, 1])
axCB = add_sub_axes(axRACMO, [0.0, -0.75, 1, 0.075])
cb = plt.colorbar(s, cax=axCB, orientation='horizontal', ticks=[0,1])
cb.set_alpha(1) # https://stackoverflow.com/questions/4478725/
cb.draw_all()
cb.ax.xaxis.set_ticks_position('top')
...
plt.savefig('fig/runoff_MAR_RACMO_KB.png', transparent=False, bbox_inches='tight', dpi=300)
#+END_SRC

#+RESULTS:
: <ipython-input-56-766334983d5d>:116: UserWarning: This figure includes Axes that are not compatible with tight_layout, so results might be incorrect.
:   plt.savefig('fig/runoff_MAR_RACMO_KB.png', transparent=False, bbox_inches='tight', dpi=300)
...
| mean  | -102.305 | 86.4549 | 354.496 | 66.1975 | 429.251 | 41.0903 | 27.5499 |  14.6388 |
| std   |  95.3457 | 9.82746 | 76.6481 |   21.58 | 42.5868 | 5.89583 | 2.94747 |  4.58547 |
| min   | -434.602 | 11.5812 | 82.2118 | 6.57694 | 84.1386 |  8.7907 | 2.68443 | 0.779265 |
| 25%   |  -160.12 | 80.3589 |  311.91 | 64.7006 | 405.333 | 37.2432 |  26.123 |  15.0577 |
| 50%   | -91.2036 | 87.8489 | 362.007 |  74.262 | 429.959 | 40.6933 | 27.8016 |  16.4186 |
| 75%   |  -39.761 | 92.0313 | 398.331 | 79.7856 | 451.414 | 44.6182 | 29.0129 |  17.3025 |
| max   |  148.424 | 105.213 | 603.404 | 93.0328 |  504.25 | 52.3235 | 34.0366 |  20.0621 |
...
| mean  | -0.242814 | -0.415007 |  -2.0052 | -0.390133 | -1.64134 | -0.153203 |  -0.121039 |   -0.078128 |
| std   |   81.0839 |   7.35376 |  79.3727 |   6.89853 |  31.9808 |   3.58547 |    2.50087 |    0.996112 |
| min   |  -360.914 |  -61.3578 | -350.869 |  -28.0695 | -420.111 |  -43.5328 |   -18.5792 |    -6.22185 |
| 25%   |  -40.0463 |  -3.13589 | -40.4169 |  -3.35965 | -3.60679 | -0.924105 |   -1.02823 |   -0.424349 |
| 50%   |  -4.10018 | -0.110043 | -3.86164 | -0.126352 | 0.855534 |  0.134652 | -0.0121199 | -0.00647888 |
| 75%   |   42.1178 |    2.9268 |  36.4895 |   2.88638 |  4.72267 |  0.932234 |   0.910387 |     0.42807 |
| max   |    347.72 |   16.7093 |  342.639 |   27.4111 |  22.7996 |   10.8596 |     7.9449 |     1.78975 |
...
* Testing
** Kjeldsen 2015 vs. Box 2013

+ Kjeldsen expands Box to include peripheral glaciers.
+ We want to un-do that scaling, but include the error estimate and Y2011 & Y2012 from Kjeldsen
...
b = pd.read_csv(datadir+'Box_2013/Greenland_mass_balance_1840-2011_Box_and_Colgan_2013.txt', delim_whitespace=True, **kw)
k = pd.read_csv(datadir+'Box_2013/Greenland_mass_balance_totals_1840-2012_ver_20141130_with_uncert_via_Kjeldsen_et_al_2015.csv', **kw)
...
#+RESULTS:
:RESULTS:
: Index(['Snow', 'Accu', 'Melt', 'A_int', 'Runof', 'SMB', 'Disc', 'TMB', 'T_mean', 'PDD_t'], dtype='object')
: Index(['accumulation', 'accumulation 1sigma', 'melt', 'melt 1sigma', 'retention',
:        'retention 1sigma', 'runoff', 'runoff 1sigma',
:        'discharge from 6 year lagged average runoff', 'discharge 1sigma', 'TMB', 'TMB 1sigma'],
:       dtype='object')
: <AxesSubplot:>
:END:

** This Study VHD vs. Box 2013 runoff
...
ru = box['runoff']
vhd = bmb['VHD_sector'].sum(dim='sector').to_dataframe(name='VHD').resample('YS').sum()
...
comp = pd.DataFrame(ru).merge(vhd, left_index=True, right_index=True)
comp.head()
# ax.scatter(comp['runoff']*0.034, comp['VHD'])
# ax.set_xlabel('Runoff (3.4%)')
# ax.set_ylabel('VHD')
...
|                     | runoff |     VHD |
|---------------------+--------+---------|
| 1986-01-01 00:00:00 | 241.19 | 5.23124 |
| 1987-01-01 00:00:00 | 294.23 | 7.90287 |
| 1988-01-01 00:00:00 | 269.36 | 6.84413 |
| 1989-01-01 00:00:00 |  221.4 | 7.11019 |
| 1990-01-01 00:00:00 | 287.23 | 7.21645 |

** VHD melting additional percent
