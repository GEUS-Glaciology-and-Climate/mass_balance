...
r.to.vect input=sectors_e@Zwally_2012 output=sectors_e type=area
v.type input=sectors_e output=sectors_e_lines from_type=boundary to_type=line
v.type input=sectors_e output=sectors_e_lines from_type=boundary to_type=line

r.to.vect input=regions_e@Mouginot_2019 output=regions_e type=area

# r.grow input=mask_ice radius=-5 output=mask_ice_shrink
# r.mask mask_ice_shrink

# v.to.rast input=sectors_e output=sectors_e_nocoast type=area use=val val=1
# r.to.vect input=sectors_e_nocoast output=sectors_e_nocoast type=line

r.to.vect input=mask_ice_shrink output=mask_ice_shrink type=area
v.clip input=sectors_e clip=mask_ice_shrink output=sectors_e_nocoast

r.to.vect input=sectors_e@Zwally_2012 output=sectors_e type=line
#+END_SRC
...
for r in mb['region'].values:
    ...
    mb_r['smb_ROI'].to_dataframe(name='M2021')\
                   .resample('AS')\
                   .sum()\
                   .rename(columns={'M2021':'SMB'})\
                   .plot(color='b', linestyle=':', **kw)
    ...
    mb_r['mb_ROI'].to_dataframe(name='M2021')\
                  .resample('AS')\
                  .sum()\
                  .rename(columns={'M2021':'MB'})\
                  .plot(color='k', linestyle='-', **kw)
