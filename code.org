...
# Install necessary system packages, but exclude GRASS GIS initially
...
# Generate and set locale (adjust as needed)
...
# Ensure /etc/default/locale is set correctly
...
# Install GRASS GIS after setting up Conda
RUN apt-get update && \
    apt-get install -y software-properties-common && \
    add-apt-repository ppa:ubuntugis/ppa && \
    apt-get update && \
    apt-get install -y grass-dev grass-gui


# Install GRASS GIS after setting up Conda
RUN apt-get install -y grass-dev grass-gui

# Set PATH and PYTHONPATH for GRASS GIS
ENV PATH="/usr/lib/grass78/bin:/usr/lib/grass78/scripts:$PATH"
ENV PYTHONPATH=/usr/lib/grass78/etc/python:$PYTHONPATH

# Install grass add-ons using the correct environment
RUN grass -e -c EPSG:3413 $HOME/G_tmp && \
    grass $HOME/G_tmp/PERMANENT --exec g.extension r.stream.basins url=https://trac.osgeo.org/grass/browser/grass-addons/grass7/raster/r.stream.basins && \
    grass $HOME/G_tmp/PERMANENT --exec g.extension r.stream.distance url=https://trac.osgeo.org/grass/browser/grass-addons/grass7/raster/r.stream.distance
