...
smb['smb'] = smb['smb_sector'].sum(dim='sector')
...
#+begin_example
<xarray.Dataset>
Dimensions:     (region: 7, sector: 19, time: 12840)
Coordinates:
  ,* time        (time) datetime64[ns] 1986-01-01 1986-01-02 ... 2021-02-25
...
#+BEGIN_SRC jupyter-python
<<load_box>>
box = box.loc['1840':'1986']
# box = box.loc['1840':'1986'].resample('1D').ffill().iloc[:-1] # cut off 1986-01-01
box = box.loc['1840':'1986'].iloc[:-1] # cut off 1986-01-01
...
smb = (365*mb['smb']).to_dataframe(name='M2021')\
                     .rename(columns={'M2021':'SMB'})\
                     .resample('AS')\
                     .mean()
smb.plot(color='b', linestyle=':', **kw)
ax.fill_between(smb.index.values,
                (smb-smb*0.15).values.flatten(),
                (smb+smb*0.15).values.flatten(),
                color='b', alpha=0.1,
                step='post')


mmbbmb = (365*(-1*(mb['mmb'] + mb['bmb']*1))).to_dataframe(name='M2021')\
                                             .rename(columns={'M2021':'MMB + BMB'})\
                                             .resample('AS')\
                                             .mean()
mmbbmb.plot(color='gray', linestyle='--', **kw)
ax.fill_between(mmbbmb.index.values,
                (mmbbmb-mmbbmb*0.1).values.flatten(),
                (mmbbmb+mmbbmb*0.1).values.flatten(),
                color='k', alpha=0.1,
                step='post')
...
# err: 10 % for MMB, 15 % for SMB. Assume random, then
mb_ann_err = sqrt((smb.values*0.15)**2 + (mmbbmb.values*0.1)**2).flatten()
ax.fill_between(mb_ann.index.values,
                (mb_ann.values.flatten()-mb_ann_err),
                (mb_ann.values.flatten()+mb_ann_err),
                color='k', alpha=0.5,
                step='post')
...
M2021 = mb['mb'].to_dataframe(name='mb_cum').resample('1D').ffill().cumsum()
M2021.loc[:'1985'] = M2021.loc[:'1985'].resample('Y').mean().resample('1D').bfill()

# (((mb['smb'].resample({'time':'YS'}).sum()*0.15)**2 + (mb['mmb'].resample({'time':'YS'}).sum()*0.1)**2)**0.5).to_dataframe(name='err').describe()
# mean: 68

OFFSET = M2021['mb_cum'].resample('YS').mean().max()
M2021['mb_cum'] = M2021['mb_cum'] - OFFSET

tt = (M2021.index.max() - M2021.index) / (M2021.index.max() - pd.Timestamp('2000-01-01'))
tt = tt.where(tt <= 1, 1)

M2021['bumpy'] = M2021['mb_cum']
M2021['smooth'] = M2021['bumpy'].rolling(365).mean()
M2021['err_core'] = (M2021['bumpy'] * (1-tt)) + (M2021['smooth'] * tt)
# M2021['err_core'] = M2021['mb_cum']

M2021['err'] = (M2021['err_core'] * 0 + 68/365)[::-1].cumsum()[::-1]
p_M2021 = M2021['mb_cum'].plot(ax=ax, color=C_M2021, drawstyle='steps-post', **kw)
# p_M2021 = M2021['mb_cum'].loc['1986':].plot(ax=ax, color=C_M2021, **kw)
# _ = M2021['mb_cum'].loc[:'1986'].resample('YS').mean().plot(ax=ax, color=C_M2021, drawstyle='steps', **kw)
# _ = M2021['mb_cum'].loc[:'1986'].plot(ax=ax, color=C_M2021, drawstyle='steps', **kw)
ax.fill_between(M2021.index,
                (M2021['err_core'] - M2021['err']).values.flatten(),
                (M2021['err_core'] + M2021['err']).values.flatten(),
...
p_gmb = grace['plot'].plot(ax=ax, label='GMB', color=C_GMB, **kw)
...
p_SEC = sec_sum.plot(ax=ax, label='VC', color=C_SEC, **kw)
...
label = ['This Study', 'M2019', 'GMB', 'VC', 'IMBIE', 'C2019', 'K2015']
...
#+RESULTS:
: <ipython-input-78-e62a8decfc25>:152: UserWarning: This figure includes Axes that are not compatible with tight_layout, so results might be incorrect.
:   plt.savefig('tmp/mouginot_2019.png', transparent=False, bbox_inches='tight', dpi=300)
...
#+RESULTS:
: <ipython-input-79-1d2631d8a6bf>:152: UserWarning: This figure includes Axes that are not compatible with tight_layout, so results might be incorrect.
:   plt.savefig('tmp/colgan_2019.png', transparent=False, bbox_inches='tight', dpi=300)
...
if 'grace' not in locals():
    <<load_grace>>
    grace = grace['mass'].resample('1D').mean().interpolate(method='time').resample('YS').mean().diff().dropna()
