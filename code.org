...
#+begin_example
<xarray.Dataset>
Dimensions:        (basin: 252, region: 7, sector: 19, time: 12823)
Coordinates:
  ,* time           (time) datetime64[ns] 1986-01-01 1986-01-02 ... 2021-02-08
...
for ROI in sectior region basin; do
  log_info MAR ${ROI}
...
#+RESULTS:|                                     |                       |     |         |      |    |           |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |                                     |        |     |        |
| $                                   | $                     | $   | $       | $    | $  | $         | $ | $ | $ | $ | $ | $ | $ | $ | $ | $ | $ | $ | > | > | > | > | [0;32m[2021-01-21T09:09:48-08:00] | [INFO] | MAR | Z[0m |
| bash:                               | ./tmp/BMB_VHD_sector.bsv:  | No  | such    | file | or | directory |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |                                     |        |     |        |
| bash:                               | ./tmp/BMB_VHD_sector.bsv:  | No  | such    | file | or | directory |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |                                     |        |     |        |
| [0;32m[2021-01-21T09:09:48-08:00] | [INFO]                | MAR | Mb[0m |      |    |           |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |                                     |        |     |        |
| bash:                               | ./tmp/BMB_VHD_basin.bsv: | No  | such    | file | or | directory |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |                                     |        |     |        |
| bash:                               | ./tmp/BMB_VHD_basin.bsv: | No  | such    | file | or | directory |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |                                     |        |     |        |
| [0;32m[2021-01-21T09:09:48-08:00] | [INFO]                | MAR | Mr[0m |      |    |           |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |                                     |        |     |        |
| bash:                               | ./tmp/BMB_VHD_region.bsv: | No  | such    | file | or | directory |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |                                     |        |     |        |
| bash:                               | ./tmp/BMB_VHD_region.bsv: | No  | such    | file | or | directory |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |   |                                     |        |     |        |
...
bmb["sector"] = pd.read_csv("./tmp/BMB_GF_sector.bsv", delimiter="|", nrows=0, index_col=0).columns.astype(np.int)

rstr = {11:'NW', 12:'NO', 1:'NE', 3:'CE', 9:'CW', 5:'SE', 7:'SW'}
rnum = pd.read_csv("./tmp/BMB_GF_region.bsv", delimiter="|", nrows=0, index_col=0).columns.astype(np.int)
bmb["region"] = [rstr[n] for n in rnum]

bmb["basin"] = pd.read_csv("./tmp/BMB_GF_basin.bsv", delimiter="|", nrows=0, index_col=0).columns.astype(np.int)



bmb['GF_sector'] = (('sector'), pd.read_csv("./tmp/BMB_GF_sector.bsv", delimiter="|", index_col=0))
bmb['vel_sector'] = (('sector'), pd.read_csv("./tmp/BMB_vel_sector.bsv", delimiter="|", index_col=0))
bmb['VHD_sector'] = (('time','sector'), pd.read_csv("./tmp/BMB_VHD_sector.bsv", delimiter="|", index_col=0, parse_dates=True).reindex(time))


GF_region = pd.read_csv("./tmp/BMB_GF_region.bsv", delimiter="|", index_col=0)
vel_region = pd.read_csv("./tmp/BMB_vel_region.bsv", delimiter="|", index_col=0)
VHD_region = pd.read_csv("./tmp/BMB_VHD_region.bsv", delimiter="|", index_col=0, parse_dates=True).reindex(time)
bmb["BMB_region"] = (("time", "region"), VHD_region + GF_region.values + vel_region.values)

GF_basin = pd.read_csv("./tmp/BMB_GF_basin.bsv", delimiter="|", index_col=0)
vel_basin = pd.read_csv("./tmp/BMB_vel_basin.bsv", delimiter="|", index_col=0)
VHD_basin = pd.read_csv("./tmp/BMB_VHD_basin.bsv", delimiter="|", index_col=0, parse_dates=True).reindex(time)
bmb["BMB_basin"] = (("time", "basin"), VHD_basin + GF_basin.values + vel_basin.values)

GF_region = pd.read_csv("./tmp/BMB_GF_region.bsv", delimiter="|", index_col=0, parse_dates=True).reindex(time)
GF_region.columns = [rstr[int(n)] for n in GF_region.columns]

GF_basin = pd.read_csv("./tmp/BMB_GF_basin.bsv", delimiter="|", index_col=0, parse_dates=True).reindex(time)


vel_region = pd.read_csv("./tmp/BMB_vel_region.bsv", delimiter="|", index_col=0, parse_dates=True).reindex(time)
vel_region.columns = [rstr[int(n)] for n in GF_region.columns]

vel_basin = pd.read_csv("./tmp/BMB_vel_basin.bsv", delimiter="|", index_col=0, parse_dates=True).reindex(time)


    
# def smb = bsv2nc(smb, (bsv, dim):
#     df = pd.read_csv("./tmp/" + bsv + ".bsv", delimiter="|", index_col=0, parse_dates=True).reindex(time)
#     df.columns = df.columns.astype(np.int64)
#     if dim == "region":
#         df.columns = [rstr[n] for n in df.columns]
...
# smb = bsv2nc(smb, ("VHD_sector", "sector")
# smb = bsv2nc(smb, ("GF_region", "region")
# smb = bsv2nc(smb, ("GF_basin", "basin")
...
<ipython-input-24-617fc7827a54> in <module>
     35 vel_basin = pd.read_csv("./tmp/BMB_vel_basin.bsv", delimiter="|", index_col=0)
     36 VHD_basin = pd.read_csv("./tmp/BMB_VHD_basin.bsv", delimiter="|", index_col=0, parse_dates=True).reindex(time)
---> 37 bmb["BMB_basin"] = (("time", "basin"), VHD_basin + GF_basin.values + vel_basin.values)
     38 
     39 GF_region = pd.read_csv("./tmp/BMB_GF_region.bsv", delimiter="|", index_col=0, parse_dates=True).reindex(time)

~/local/miniconda3/envs/TMB/lib/python3.8/site-packages/pandas/core/ops/__init__.py in f(self, other, axis, level, fill_value)
...
~/local/miniconda3/envs/TMB/lib/python3.8/site-packages/pandas/core/ops/__init__.py in _align_method_FRAME(left, right, axis, flex, level)
    488 
    489             else:
...
ValueError: Unable to coerce to DataFrame, shape must be (12823, 191): given (1, 252)
#+end_example
:END:
...
for roi in ['sector', 'region', 'basin']:
    ...
    for RCM in ['HIRHAM', 'MAR', 'RACMO', 'mb']:
        ...
        mb['mb_HIRHAM'] = (('time',roi), smb['HIRHAM_'+roi]-mmb['mmb_'+roi] )
        mb['mb_MAR'] = (('time',roi), smb['HIRHAM_'+roi]-mmb['mmb_'+roi])
        mb['mb_RACMO'] = (('time',roi), smb['HIRHAM_'+roi]-mmb['mmb_'+roi])
        for v in ['mb_HIRHAM','mb_MAR','mb_RACMO']:
...
mb['mb'].to_dataframe(name='M2021')\
        .rename(columns={'M2021':'MB'})\
        .resample('AS')\
        .sum()\
        .plot(color='k', linestyle='-', **kw)
...
mb['mb'].to_dataframe(name='M2021')\
        .rename(columns={'M2021':'MB'})\
        .plot(ax=ax2, color='k', linestyle='-', legend=False, drawstyle='steps-post')
...
#+RESULTS:
: <ipython-input-189-1b9b56ec5892>:92: UserWarning: FixedFormatter should only be used together with FixedLocator
:   axr.set_yticklabels(np.round(np.abs(axr.get_yticks()),2))
** TODO TMB vs. others timeseries: cumsum FORWARD
...
hexcolor = map(lambda rgb:'#%02x%02x%02x' % (np.int(rgb[0]*255),np.int(rgb[1]*255),np.int(rgb[2]*255)),
               tuple(color[:,0:-1]))
mpl.rcParams['axes.prop_cycle'] = plt.cycler('color' , hexcolor)
...
mb['HIRHAM_sector'].sum(dim='sector').to_dataframe(name='M2021 (HIRHAM)').cumsum().plot(ax=ax)
mb['MAR_sector'].sum(dim='sector').to_dataframe(name='M2021 (MAR)').cumsum().plot(ax=ax, linestyle='--')
mb['RACMO_sector'].sum(dim='sector').to_dataframe(name='M2021 (RACMO)').cumsum().plot(ax=ax)
...
# Mouginot
<<load_mouginot>>
M = mouginot['MB'].sum(dim='region').to_dataframe('M2019').cumsum()
M_err = mouginot['MB_err'].sum(dim='region').to_dataframe('M2019').cumsum()
...
** SUCCESS This vs. Colgan (2019): GIS xy
...
def my_plot(ax, x, y):
    ...
    if ax == axSMB:
        ...
        ax.set_xticks([0,600])
        ax.set_xlabel('C2019 [Gt yr$^{-1}$]')
        ax.set_ylabel('M2021 [Gt yr$^{-1}$]')
    if ax == axD:
...
my_plot(axSMB, promice['SMB'].sum(dim='sector'), this_smb['smb'])
my_plot(axD,   promice['D'].sum(dim='sector'), this_D['mmb'])
s = my_plot(axMB,  promice['MB'].sum(dim='sector'), this_mb['mb'])
...
#+RESULTS:
: <ipython-input-111-524d8b1db768>:191: UserWarning: This figure includes Axes that are not compatible with tight_layout, so results might be incorrect.
:   plt.savefig('tmp/colgan_2019.png', transparent=False, bbox_inches='tight', dpi=300)

[[./tmp/colgan_2019.png]]


** SUCCESS This vs. Mouginot (2019): GIS xy
...
#+RESULTS:
: <ipython-input-112-58c68da1343f>:191: UserWarning: This figure includes Axes that are not compatible with tight_layout, so results might be incorrect.
:   plt.savefig('tmp/mouginot_2019.png', transparent=False, bbox_inches='tight', dpi=300)


[[./tmp/mouginot_2019.png]]

** INPROGRESS This vs. IMBIE GIS XY
...
# gs = gridspec.GridSpec(1, 1) #w,h
# ax = plt.subplot(gs[:,:])
axGRACE = fig.add_subplot(131)
axSEC = fig.add_subplot(132)
axIMBIE = fig.add_subplot(133)
...
my_plot(axGRACE, gmb, this_mb_gmb)
my_plot(axSEC, sec, this_mb_sec)
s = my_plot(axIMBIE,  imbie, this_mb_imbie)
...
# axCB = add_sub_axes(axIMBIE, [1.1, 0.0, 0.075, 1])
# cb = plt.colorbar(s, cax=axCB, orientation='vertical', ticks=[0,1])
# cb.set_alpha(1) # https://stackoverflow.com/questions/4478725/
# cb.draw_all()
# # cb.set_label("Time [year]", labelpad=-20)
