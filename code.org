...
for v in ['D_sector','D_region','D']:
    ...
    _, p1 = detrend_dim(season.isel({'time':slice(0,last3_break[0])}), 'time')
    _, p2 = detrend_dim(season.isel({'time':slice(last3_break[0],last3_break[1])}), 'time')
    _, p3 = detrend_dim(season.isel({'time':slice(last3_break[1],999)}), 'time')
    ...
    season_trend = xr.polyval(xr.ones_like(forecast)['time'], ((p1+p2+p3)/3).polyfit_coefficients)
    season_trend = season_trend - season_trend.isel({'time':0}) + season_trend.diff(dim='time').isel({'time':0})
    forecast = forecast + season_trend
    
    forecast1 = forecast.where(forecast['time'] == forecast['time'][-1]).dropna(dim='time')
    D_fc[v] = xr.concat([D[v],forecast1], dim='time').interp(time=time_index_all)
    
for v in ['D_err','err_sector','err_region']:
    last3_hindcast = D_fc[v].sel({'time':time_index_hindcast})
    diff = np.abs(last3_hindcast.reindex({'time':time_index_last3y}).diff(dim='time').dropna(dim='time'))

    if np.size(diff.dims) == 1:
        err = diff.sortby(diff, ascending=False).values[0:time_index_forecast.size]
    ...
    D_fc[v] = D_fc[v].ffill(dim='time') + e.cumsum().reindex({'time':time_index_all}).fillna(0)
...
# D_fc['D'].plot(ax=ax)
# ax.fill_between(D_fc['time'],
#                 D_fc['D']-D_fc['D_err'],
#                 D_fc['D']+D_fc['D_err'],
#                 color='gray', alpha=0.25)

r = 'NO'
D_fc['D_region'].sel({'region':r}).plot(ax=ax)
ax.fill_between(D_fc['time'],
                D_fc['D_region'].sel({'region':r}) - D_fc['err_region'].sel({'region':r}),
                D_fc['D_region'].sel({'region':r}) + D_fc['err_region'].sel({'region':r}),
                color='gray', alpha=0.25)
...
if this.index.size > 8:
    this['MB'].cumsum().iloc[:-10].plot(ax=ax, label='2022 (through '+last_day+')', **kw_plot)
    kw_plot['alpha'] = 0.8
